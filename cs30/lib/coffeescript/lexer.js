...
export Lexer = class Lexer {
  ...
  clean(code) {
    ...
    if (WHITESPACE.test(code)) {
      ...
      this.locationDataCompensations[0] = this.locationDataCompensations[0] - 1;
    }
    ...
    if (prev && indexOf.call(['=', ...COMPOUND_ASSIGN], value) >= 0) {
      ...
      if (value === '=' && ((ref = prev[1]) === '||' || ref === '&&') && !prev.spaced) {
        ...
        if ((ref1 = prev.data) != null ? ref1.original : void 0) {
          prev.data.original = prev.data.original + '=';
        }
        prev[2].range = [prev[2].range[0], prev[2].range[1] + 1];
        prev[2].last_column = prev[2].last_column + 1;
        prev[2].last_column_exclusive = prev[2].last_column_exclusive + 1;
  ...
  matchWithInterpolations(regex, delimiter, closingDelimiter = delimiter, interpolators = /^#\{/) {
    ...
    while (true) {
      ...
      offsetInChunk = offsetInChunk + strPart.length;
      ...
      str = str.slice(index);
      offsetInChunk = offsetInChunk + index;
    }
      ...
      switch (tag) {
        ...
        case 'NEOSTRING':
          ...
          if (tokens.length === 1 && (quote != null)) {
            token[2].first_column = token[2].first_column - quote.length;
            if (token[1].substr(-2, 1) === '\n') {
              token[2].last_line = token[2].last_line + 1;
              token[2].last_column = quote.length - 1;
            } else {
              token[2].last_column = token[2].last_column + quote.length;
              if (token[1].length === 2) {
                token[2].last_column = token[2].last_column - 1;
              }
            }
            token[2].last_column_exclusive = token[2].last_column_exclusive + quote.length;
