...
export compile = withPrettyErrors (code, options = {}) ->
  ...
  # Phase 5a: Unwrap simple IIFEs from comprehensions converted to array methods
  # Pattern: (() => { let results; arrayMethod(...) })() → arrayMethod(...)
  if process?.env?.ES6
    # Match IIFE wrapping array methods with balanced parentheses check
    # CoffeeScript's compileClosure generates: (() => { let results; ... })()
    js = js.replace /\(\(\) => \{[\s\n]+(let [^;]+;[\s\n]+)?(.+?\.(map|filter)\(.+?\))[\s\n]+\}\)\(\)/gs, (match, vars, method) ->
      # Only unwrap if the method has balanced parentheses
      openParens = (method.match(/\(/g) or []).length
      closeParens = (method.match(/\)/g) or []).length
      if openParens is closeParens then method else match

    # TODO: Single parameter optimization - disabled due to test file conflicts
    # Would transform: (x) => expr  →  x => expr
    # Issue: Regex also matches test file strings, corrupting test expectations
    # Solution needed: More sophisticated string-aware processing
    # js = js.replace /(^|[=,(]\s*)\(([a-z_$][a-z0-9_$]*)\)\s*=>/gim, '$1$2 =>'
...
export patchStackTrace = ->
  ...
  # positions. #!# FIXME: Can we do this better now?
