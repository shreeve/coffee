...
const isUnassignable = function(name, displayName = name) {
  switch (false) {
    case !([...JS_KEYWORDS, ...COFFEE_KEYWORDS].includes(name)):
...
export class Lexer {
  ...
  identifierToken() {
    ...
    if (id === 'as' && this.seenImport) {
      ...
      } else if (ref = this.value(true), COFFEE_KEYWORDS.includes(ref)) {
        prev = this.prev();
    ...
    if (id === 'as' && this.seenExport) {
      ...
      if (ref3 = this.value(true), COFFEE_KEYWORDS.includes(ref3)) {
        prev = this.prev();
        [prev[0], prev[1]] = ['IDENTIFIER', this.value(true)];
        this.token('AS', id);
        return id.length;
    ...
    let tokenData = {};
    let prevprev;
    if (tag === 'IDENTIFIER' && (JS_KEYWORDS.includes(id) || COFFEE_KEYWORDS.includes(id)) && !(this.exportSpecifierList && COFFEE_KEYWORDS.includes(id))) {
      tag = id.toUpperCase();
...
/* DEFPART_MARKER */
  ...
  commentToken(chunk = this.chunk, {heregex, returnCommentTokens = false, offsetInChunk = 0} = {}) {
    ...
    let matchIllegal, content, leadingNewlines, precedingNonCommentLines, hasSeenFirstCommentLine;
    if (hereComment) {
...
/* DEFPART_MARKER */
      for (let i = j = 0, len = contents.length; j < len; i = ++j) {
        let {content, length, leadingWhitespace, precededByBlankLine} = contents[i];
        let nonInitial = i !== 0;
...
/* DEFPART_MARKER */
      ...
      case !(match = REGEX.exec(this.chunk)):
        ...
        let prev = this.prev();
        if (prev) {
          if (prev.spaced && (ref = prev[0], CALLABLE.includes(ref))) {
            if (!closed || POSSIBLY_DIVISION.test(regex)) {
              return 0;
            }
          } else if (ref1 = prev[0], NOT_REGEX.includes(ref1)) {
            return 0;
          }
        }
        if (!closed) {
  ...
  lineToken({chunk = this.chunk, offset = 0} = {}) {
    ...
    let diff, endsContinuationLineIndentation;
    if (size > this.indent) {
      if (noNewlines) {
        if (!backslash) {
  ...
  literalToken() {
    ...
    let tag = value;
    let prev = this.prev();
    let skipToken, origin, message;
    if (prev && ['=', ...COMPOUND_ASSIGN].includes(value)) {
      skipToken = false;
    ...
    if (value === ';') {
      if (ref3 = prev != null ? prev[0] : void 0, ['=', ...UNFINISHED].includes(ref3)) {
      ...
      if (value === '(' && !prev.spaced && (ref4 = prev[0], CALLABLE.includes(ref4))) {
        ...
        tag = 'CALL_START';
      } else if (value === '[' && (((ref5 = prev[0], INDEXABLE.includes(ref5)) && !prev.spaced) || (prev[0] === '::'))) {
        tag = 'INDEX_START';
        switch (prev[0]) {
          case '?':
            prev[0] = 'INDEX_SOAK';
...
/* DEFPART_MARKER */
  ...
  pair(tag) {
    let ref = this.ends, [prev] = slice.call(ref, -1);
    if (tag !== (wanted = prev != null ? prev.tag : void 0)) {
      if ('OUTDENT' !== wanted) {
        this.error(`unmatched ${tag}`);
      }
      let ref1 = this.indents, [lastIndent] = slice.call(ref1, -1);
      this.outdentToken({
        moveOut: lastIndent,
        noNewlines: true
      });
  ...
  getLineAndColumnFromChunk(offset) {
    ...
    let previousLinesCompensation, columnCompensation;
    if (lineCount > 0) {
  ...
  tag() {
    let ref = this.tokens, [token] = slice.call(ref, -1);
  ...
  validateEscapes(str, options = {}) {
    ...
    let before, octal, hex, unicodeCodePoint, unicode; match[0], before = match[1], octal = match[2], hex = match[3], unicodeCodePoint = match[4], unicode = match[5];
  ...
  suppressSemicolons() {
    ...
    while (this.value() === ';') {
      this.tokens.pop();
      if (ref = (ref1 = this.prev()) != null ? ref1[0] : void 0, ['=', ...UNFINISHED].includes(ref)) {
